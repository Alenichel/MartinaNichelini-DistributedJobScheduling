...
\begin{itemize}
	...
	\item There is not a hierarchy between executors, they join the executors "network" by auto-discovering all other running executors.
	\item All executors can be contacted by clients. A client needs to know the address of a executor to be able to connect.
	\item The system uses both UDP and TCP sockets for communications between executors and it exploits broadcast UDP packets for auto-discovering. Thus the system is able to run in local networks inside a single broadcast domain.
...
\end{lstlisting}
\end{file}
	\item New job requests are collected by the connected executor and forwarded to the executor with the shortest job queue. Executors are multithreading, by default they handle two jobs at a time.
	...
	\item Job results are kept only by the actual executors. All others have a reference to it.
